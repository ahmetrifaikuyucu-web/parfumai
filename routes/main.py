...
def survey():
    ...
    return render_template('survey.html', gender_question=gender_q, questions=regular_qs, csrf_token=generate_csrf_token())  # noqa: E501
...
def benzer(name):
    ...
    if isinstance(top_notes, list):
        top_notes = ', '.join(top_notes[:3])
    mid_notes = source.get('notes_middle', [])
    if isinstance(mid_notes, list):
        mid_notes = ', '.join(mid_notes[:3])
    base_notes = source.get('notes_base', [])
    if isinstance(base_notes, list):
        base_notes = ', '.join(base_notes[:3])
    ...
    try:
        ...
        generate_explanations(note_profile, recommendations, 'unisex', decoded)
    except Exception:
        pass
