...
from perfume_engine import _init_database, save_database  # noqa: E402
...
def _make_img(w, h, nr, ng, nb):
    ...
    for y in range(h):
        _ = int(180 - 80*abs(y-h/2)/(h/2))
        for x in range(4):
            px0 = img.getpixel((x, y))
            img.putpixel((x, y), (min(255, px0[0]*0.3+nr*0.7), min(255, px0[1]*0.3+ng*0.7), min(255, px0[2]*0.3+nb*0.7), 255))  # noqa: E501
    cx, cy = w//2, h//2-10
        ...
        for dx in range(-100, 101):
            ...
            if 0 <= px < w and 0 <= py < h:
                ...
                if ag > 0:
                    ...
                    img.putpixel((px, py), (min(255, p0[0]+int(nr*ag/255)), min(255, p0[1]+int(ng*ag/255)), min(255, p0[2]+int(nb*ag/255)), 255))  # noqa: E501
    return img, draw
...
def generate(brand, name, season):
    ...
    for p in ['C:/Windows/Fonts/arialbd.ttf', 'C:/Windows/Fonts/arial.ttf', 'C:/Windows/Fonts/segoeuib.ttf']:
        try:
            if fb is None:
                fb = ImageFont.truetype(p, 90)
            if fn is None:
                fn = ImageFont.truetype(p, 20)
        except Exception:
            pass
    if fb is None:
    ...
    for x in range(W):
        ...
        for y in range(H-3, H):
            p = img.getpixel((x, y))
            img.putpixel((x, y), (min(255, p[0]+int(nr*al/255)), min(255, p[1]+int(ng*al/255)), min(255, p[2]+int(nb*al/255)), 255))  # noqa: E501
