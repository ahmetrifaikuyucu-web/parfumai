...
- **Giriş:** `admin123` (`.env` ile değiştirin)
...
| GET | `/api/node_categories` | Nota kategorileri (JSON) |
| GET | `/api/similar/<name>` | Benzer parfümleri getir |
| GET | `/benzer/<name>` | Benzer parfüm sayfası |
| GET | `/api/stock` | Stok durumu |
| POST | `/api/log-sale` | Satış kaydet |
| GET | `/admin` | Yönetici paneli |
| GET | `/admin/login` | Admin giriş sayfası |
| GET | `/admin/logout` | Admin çıkış |
| GET | `/admin/perfumes` | Parfüm yönetimi |
| POST | `/admin/perfumes/add` | Parfüm ekle |
| POST | `/admin/perfumes/edit` | Parfüm düzenle |
| POST | `/admin/perfumes/delete` | Parfüm sil |
| POST | `/admin/perfumes/toggle-stock` | Stok durumu değiştir |
| GET | `/api/admin/audit-log` | Denetim günlüğü |
